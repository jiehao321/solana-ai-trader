...
if (!PRIVATE_KEY || !POLY_API_KEY || !POLY_SECRET || !POLY_PASSPHRASE) {
    ...
    console.error("需要: PRIVATE_KEY, POLY_API_KEY, POLY_SECRET, POLY_PASSPHRASE");
    process.exit(1);
}
...
async function main() {
    ...
    if (!args.token || !args.price || !args.size) {
        console.log("用法:");
        console.log("  bun run scripts/buy.ts --token TOKEN_ID --price 0.50 --size 100 --type limit");
        console.log("  bun run scripts/buy.ts --token TOKEN_ID --price 0.50 --size 100 --type market");
        console.log("\n参数:");
        ...
        console.log("\n注意:");
        console.log("  LIMIT 订单: size = 份额数量");
        console.log("  MARKET 订单: size = 美元金额");
        process.exit(1);
    }
    ...
    try {
        ...
        if (orderType === "market") {
            ...
            console.log("总成本:", (size * price).toFixed(2), "USDC");
        }
        ...
        if (error.message.includes("INVALID_ORDER_NOT_ENOUGH_BALANCE")) {
            ...
            console.error("原因: 订单数量太小");
        } else if (error.message.includes("INVALID_ORDER_MIN_TICK_SIZE")) {
            console.error("原因: 价格精度不对，请使用 0.001 的倍数");
        }
        process.exit(1);
    }
}
